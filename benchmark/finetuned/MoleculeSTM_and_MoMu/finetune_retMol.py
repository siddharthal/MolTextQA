...
from datasets import Mol_retrieval
...
def do_CL(X, Y, label):
    ...
    CL_loss = criterion(logits, label)
...
def get_loss(batch):
    ...
    molecule_repr = [get_molecule_repr(molecule_data[idx]) for idx in range(5)]
    molecule_repr = torch.stack(molecule_repr)

    train_loss = do_CL(text_repr, molecule_repr, label.to(device))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_model_config", type=str, default="MoMu.pth")
    ...
    dataset = Mol_retrieval(args.dataspace_path)

    # Finetune
    train_idx = np.where(np.asarray(dataset.splits) == "train")[0]
    train_set = dataset.copy(train_idx)
    dataloader_class = pyg_DataLoader
    train_loader = dataloader_class(train_set, batch_size=args.batch_size, shuffle=True, num_workers = 6)

    model_param_group = [
        {"params": text_model.parameters(), "lr": args.lr},
        {"params": molecule_model.parameters(), "lr": args.lr},
        {"params": text2latent.parameters(), "lr": args.lr},
        {"params": mol2latent.parameters(), "lr": args.lr},
        ]
    ...
    SAVE_PATH = "preds_retMol_finetune/"
    os.makedirs(SAVE_PATH, exist_ok=True) # Create directory if it does not exist
    check_dir = SAVE_PATH + args.input_model_config
    ...
    f.write("--------------------------------------------------------------------------------- \n")
    f.write("finetune_{}_lr:{}".format(args.input_model_config, args.lr))
    f.write("\n")
