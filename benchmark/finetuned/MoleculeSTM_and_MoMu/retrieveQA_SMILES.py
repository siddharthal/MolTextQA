...
from datasets import QA_retrieval_SMILES
...
def eval_epoch(dataloader):
    ...
    for batch in tqdm(dataloader):
        ...
        text_repr = [model.encode(text[idx]) for idx in range(5)]
        text_repr = torch.stack(text_repr)
        molecule_repr = model.encode(molecule_data)
...
if __name__ == "__main__":
    ...
    dataset = QA_retrieval_SMILES(args.dataspace_path)
