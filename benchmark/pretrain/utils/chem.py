...
def calc_tanimoto(fps):
    """
    Input
    fps: Fingerprint of molecules in the batch (Shape: B * 1024)
    
    Output
    Tanimoto similarity between the molecules in the batch (Shape : B * B) 
    """
    tanimoto_mat = []
    for i in range(fps.shape[0]):
        intersection = torch.logical_and(fps[i], fps)
        union = torch.logical_or(fps[i], fps)
        tanimoto_mat.append(intersection.sum(dim = 1) / union.sum(dim = 1))
    
    return torch.vstack(tanimoto_mat)


def calc_tanimoto2(fps1, fps2):
    """
    Input
    fps: Fingerprint of molecules in the batch (Shape: B * 1024)
    ...
    for i in range(fps1.shape[0]):
        intersection = torch.logical_and(fps1[i], fps2)
        union = torch.logical_or(fps1[i], fps2)
        tanimoto_mat.append(intersection.sum(dim = 1) / union.sum(dim = 1))
